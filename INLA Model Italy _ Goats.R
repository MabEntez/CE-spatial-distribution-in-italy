...
r_prev_mean <- mask(r_prev_mean, bas_map, df= TRUE)

save.image("INLAData_G.RData")

#### Model output visualisation ####
load("INLAData_G.RData")
...
m.g <- ggplot(it_map, aes(x=long,y=lat))+
...
png("Goats prev mean.png", width = 2000, height = 2000,)
m.g
dev.off()

pal <- colorNumeric("viridis", c(0.24, 0.34), na.color = "transparent")
